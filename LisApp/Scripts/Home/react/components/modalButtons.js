...
const ModalButtons = ({ mode, onCancel, actualTabIndex, tabCount, onPrev, onNext, prevId, nextId, onPrevStudy, onNextStudy, withoutPrevStudy, canFireEmployee, onFire }) => (
    <div className="save-cancel-buttons">
        {!withoutPrevStudy && prevId != null && onPrevStudy != null && mode === 'show' ? <button className="modal-buton action-modal-button" onClick={onPrevStudy}><Trans>PrevStudy</Trans></button> : null}
        {!withoutPrevStudy && nextId != null && onNextStudy != null && mode === 'show' ? <button className="modal-buton action-modal-button" onClick={onNextStudy}><Trans>NextStudy</Trans></button> : null}
        {canFireEmployee === true && mode === 'edit' ? <button className="modal-buton action-modal-button" onClick={onFire}><Trans>Fire</Trans></button> : null}
