...
const ModalButtons = ({ mode, onCancel, actualTabIndex, tabCount, onPrev, onNext, prevId, nextId, onPrevStudy, onNextStudy, withoutPrevStudy, canFireEmployee, onFire, disableSave }) => (
    <div className="save-cancel-buttons">
        ...
        {mode !== 'show' && mode !== 'addSample' ? <button className="modal-buton action-modal-button" type="submit" form="modalform" disabled={(actualTabIndex < tabCount - 1) || disableSave}>{<Trans>{mode === 'start' ? "Start" : "Save"}</Trans>}</button> : null}
        <button className="modal-buton cancel-modal-button" onClick={onCancel}>{<Trans>{mode === 'show' || mode === 'addSample' ? "Back" : "Cancel"}</Trans>}</button>
    </div>
);
